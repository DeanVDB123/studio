...
// This will now only run if the global cache is truly empty for the first time.
if (process.env.NODE_ENV === 'development' && memorials.size === 0) {
  const dummyPhoto: Photo = { id: 'dummy-photo-1', url: 'https://placehold.co/600x400.png', caption: 'A beautiful memory' };
  createMemorial({
    deceasedName: 'Jane Doe (Dev)',
    ...
    photos: [dummyPhoto, { id: 'dummy-photo-2', url: 'https://placehold.co/600x400.png', caption: 'Happy times' }],
   ...
   createMemorial({
    ...
    photos: [{id: 'dummy-photo-3', url: 'https://placehold.co/600x400.png', caption: 'Adventure time'}],
  ...
  console.log('Initialized dev data. Memorials count:', memorials.size);
}
